  ...
  static addCoupon = async (couponData) => {
    try {
      const coupon = new Coupon({
        ...couponData,
        startDate: this.toISTDate(couponData.startDate),
        endDate: this.toISTDate(couponData.endDate),
      });
  ...
  static async validateCoupon(code, cartTotal, cartItems) {
    ...
    if (coupon.minOrderAmount && cartTotal < coupon.minOrderAmount)
      throw new ValidationError(
        `Minimum cart value ${coupon.minOrderAmount} required`,
        400
      );
    if (cartTotal < 100)
